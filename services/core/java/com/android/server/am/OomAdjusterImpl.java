...
public class OomAdjusterImpl extends OomAdjuster {
    ...
    private void computeOomAdjLSP(ProcessRecordInternal app, ProcessRecordInternal topApp,
        ...
        if (app == topApp && PROCESS_STATE_CUR_TOP == PROCESS_STATE_TOP) {
            ...
            if(OomAdjuster.mIsTopAppRenderThreadBoostEnabled) {
                if(OomAdjuster.mCurRenderThreadTid != app.getRenderThreadTid() && app.getRenderThreadTid() > 0) {
                    OomAdjuster.mCurRenderThreadTid = app.getRenderThreadTid();
                    if (OomAdjuster.mPerfBoost != null) {
                        Slog.d(TAG, "TOP-APP: pid:" + app.getPid() + ", processName: "
                               + app.processName + ", renderThreadTid: " + app.getRenderThreadTid());
                        if (OomAdjuster.mPerfHandle >= 0) {
                            OomAdjuster.mPerfBoost.perfLockRelease();
                            OomAdjuster.mPerfHandle = -1;
                        }
                        OomAdjuster.mPerfHandle = OomAdjuster.mPerfBoost.perfHint(BoostFramework.VENDOR_HINT_BOOST_RENDERTHREAD,
                                                          app.processName, app.getRenderThreadTid(), 1);
                        Slog.d(TAG, "VENDOR_HINT_BOOST_RENDERTHREAD perfHint was called. mPerfHandle: "
                               + OomAdjuster.mPerfHandle);
                    }
                }
            }
